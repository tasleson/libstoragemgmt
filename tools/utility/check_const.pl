...
# Some python modules (version.py) are generated by configure into the build
# directory rather than living in the source tree, so an out of tree build has
# no copy of them next to __init__.py. --build-dir names that directory and is
# consulted only as a fallback, after the source tree.
my $BUILD_DIR;
GetOptions( 'build-dir=s' => \$BUILD_DIR )
  or die "Usage: $0 [--build-dir DIR]\n";

my $LSM_CODE_BASE_DIR = dirname( dirname( dirname( abs_path($0) ) ) );
my $PYTHON_LIB_DIR    = "$LSM_CODE_BASE_DIR/python_binding/lsm";
my $PYTHON_BUILD_LIB_DIR;
$PYTHON_BUILD_LIB_DIR = "$BUILD_DIR/python_binding/lsm" if defined $BUILD_DIR;
...
sub _resolve_py_module($$) {

    # Take the source folder holding __init__.py and a module path relative to
    # it, return the file to parse.  Generated modules are absent from the
    # source tree, so fall back to the build directory when one was given.  A
    # module missing from both is returned as its source path, so that the
    # open() of it fails loudly rather than being silently skipped.
    my $src_folder = shift;
    my $rel_path   = shift;

    my $src_file = "$src_folder/$rel_path";
    return $src_file if -e $src_file;

    if ( defined $PYTHON_BUILD_LIB_DIR ) {
        my $build_file = "$PYTHON_BUILD_LIB_DIR/$rel_path";
        return $build_file if -e $build_file;
    }
...
sub _parse_py_init_file($) {
    ...
    foreach my $line (@lines) {
        if ( $line =~ /from ([^ ]+) import (.+)$/ ) {
            ...
            push @rc1,
              _resolve_py_module( $folder_path, "$module_file_name.py" );
